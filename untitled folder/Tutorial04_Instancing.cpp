...
void Tutorial04_Instancing::UpdateUI()
{
    ...
    if (ImGui::Begin("Settings", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        ImGui::Text("xProj Value: %.3f ", xProj);
        ImGui::Text("yProj Value: %.3f ", yProj);
        if (ImGui::SliderInt("Grid Size", &m_GridSize, 1, 32))
        {
            PopulateInstanceBuffer();
        }
         if (ImGui::SliderInt("X View", &xView, -5, 5))
        {
            
        }
        if (ImGui::SliderInt("Y View", &yView, -5, 5))
       {
           
       }
        if (ImGui::SliderInt("Z View", &zView, 0, 10))
       {
       ...
       {
            xProj += -ImGui::GetIO().MouseDelta.y/100;
            yProj += -ImGui::GetIO().MouseDelta.x/100;
       }
        
       ImGui::Text("CONTROLS: drag with left mouse click to rotate x and y, scroll wheel for x,\n sliders move camera");
    }
...
void Tutorial04_Instancing::Update(double CurrTime, double ElapsedTime)
{
    ...
    float4x4 View = float4x4::RotationX(xProj) * float4x4::RotationY(yProj) * float4x4::Translation(xView, yView, zView);
